...
@testset "FHIaims" begin
    ...
    @testset "find_operatorkinds" begin
        ...
        found_operatorkinds = find_operatorkinds(root, FHIaimsOperator)
    ...
    @testset "load_operator_metadata" begin
        rs_indices = """
        ...
        setupteardown_tmp() do
            ...
            operator_metadata = load_operator_metadata(pwd(), FHIaimsOperator)
            @test operator_metadata.n_basis == 2
            @test operator_metadata.row_ind == [1, 1, 2, 1, 1, 1]
            @test operator_metadata.col_cell_ptr[1, :, :] == [1 2; 4 0; 5 6]
            @test operator_metadata.col_cell_ptr[2, :, :] == [1 3; 4 -1; 5 6]
            @test operator_metadata.cells == [SA[0, 0, 0], SA[0, 0, 1], SA[0, 0, -1]]
        end
    end
    ...
    @testset "load_operator_data" begin
        ...
        @testset "Plain" begin
            setupteardown_tmp() do
                ...
                operator_data = load_operator_data(pwd(), Hamiltonian(:ref), FHIaimsOperator)
