...
@option struct DOSParams <: AbstractQuollParams
    ...
    function DOSParams(smearing_function, temperature, n_points, energy_begin, energy_end)
        ...
        return new(smearing_function, temperature, n_points, energy_begin, energy_end)
    end
end
...
function parse_kpathparams(band::AbstractString)
    ...
    @argcheck(
        all(x -> x !== nothing, kpoint_begin) &&
            all(x -> x !== nothing, kpoint_end) &&
            n_points !== nothing,
        "K-point values in the band structure are not parsable"
    )
    ...
    return KPathParams(
        SVector{3,Float64}(kpoint_begin),
        SVector{3,Float64}(kpoint_end),
        n_points,
        symbol_begin,
        symbol_end,
    )
